...
interface QuizFastingInfoScreenProps {
  onNext: (digestion: string) => void;
  onBack: () => void;
}
...
export const QuizFastingInfoScreen: React.FC<QuizFastingInfoScreenProps> = ({ onNext, onBack }) => {
  const options = [
    { id: 'bloated', label: 'Súper hinchada y pesada', emoji: '🎈' },
    { id: 'normal_low_energy', label: 'Normal, pero con poca energía', emoji: '🔋' },
    { id: 'depends', label: 'Depende mucho de lo que coma', emoji: '🤔' },
    { id: 'good', label: 'Bastante bien, la verdad', emoji: '👍' },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-6 flex flex-col items-center text-center flex-grow pb-8">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold text-[#004D40] leading-tight mb-8">
          Perfecto. ¿Cómo sentís la panza al final del día después de toda tu rutina?
        </h2>
