...
interface QuizHeightScreenProps {
...
export const QuizHeightScreen: React.FC<QuizHeightScreenProps> = ({ onNext, onBack }) => {
  ...
  const handleCmChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length <= 3) setCm(value);
  };

  const handleFtChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value.replace(/[^0-9]/g, '');
    if (value.length <= 1) setFt(value);
  };

  const handleIncChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value.replace(/[^0-9]/g, '');
    if (value.length <= 2) setInc(value);
  };
  ...
  const isValid = () => {
    if (unit === 'cm') {
      ...
      const f = parseInt(ft);
      const i = parseInt(inc);
      return f >= 1 && f <= 9 && !isNaN(i) && i >= 0 && i < 12;
    }
  };
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (isValid()) {
      const finalValue = unit === 'cm' ? `${cm} cm` : `${ft} ft ${inc} in`;
      onNext(finalValue);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-4 mt-4 mb-6 flex items-end space-x-3">
        ...
        <ProgressBar currentStep={21} totalSteps={46} />
      </div>
            ...
            {unit === 'cm' ? (
              <div className="flex items-center justify-center">
                <input
                  ...
                  autoFocus
                />
                <span className="text-xl text-[#004D40] font-medium ml-2">cm</span>
              </div>
            ) : (
              <div className="flex items-center justify-center space-x-4">
                <div className="flex items-center">
                  <input
                    ...
                    placeholder="5"
                    className="w-16 text-center text-3xl text-[#004D40] font-bold placeholder-slate-300 outline-none bg-transparent"
                    autoFocus
                  />
                  <span className="text-xl text-[#004D40] font-medium ml-1">ft</span>
                </div>
                <div className="flex items-center">
                  <input
                    type="tel"
                    ...
                    className="w-16 text-center text-3xl text-[#004D40] font-bold placeholder-slate-300 outline-none bg-transparent"
                  />
                  <span className="text-xl text-[#004D40] font-medium ml-1">in</span>
                </div>
              </div>
            )}
          </div>
