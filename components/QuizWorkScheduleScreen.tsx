...
interface QuizWorkScheduleScreenProps {
  onNext: (schedule: string) => void;
  onBack: () => void;
}
...
export const QuizWorkScheduleScreen: React.FC<QuizWorkScheduleScreenProps> = ({ onNext, onBack }) => {
  const options = [
    { id: 'office', label: 'Horario fijo de oficina', emoji: '🏢' },
    { id: 'wfh_flexible', label: 'Trabajo desde casa o flexible', emoji: '🏠' },
    { id: 'rotating', label: 'Turnos rotativos', emoji: '🔄' },
    { id: 'entrepreneur', label: 'Emprendedora, no tengo horario fijo', emoji: '🚀' },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-6 flex flex-col items-center text-center flex-grow pb-8">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold text-[#004D40] leading-tight mb-8">
          Anotado. ¿Cómo es tu rutina laboral o de ocupaciones?
        </h2>
