...
interface QuizThankYouComplianceScreenProps {
  userName: string;
  onNext: () => void;
  onBack: () => void;
}
...
export const QuizThankYouComplianceScreen: React.FC<QuizThankYouComplianceScreenProps> = ({ userName, onNext, onBack }) => {
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-4 mt-4 mb-6 flex items-end space-x-3">
        ...
        <ProgressBar currentStep={34} totalSteps={46} />
      </div>
      ...
      <div className="w-full max-w-md px-6 flex flex-col items-center text-center pb-8">
        ...
        <div className="w-full bg-white rounded-2xl p-8 shadow-sm border border-slate-50 mt-4 mb-10">
            {/* Headline */}
            <h2 className="text-2xl sm:text-3xl font-bold text-[#004D40] leading-tight mb-6">
              Gracias {userName} 💙
            </h2>

            {/* Explanation/Safety Text */}
            <p className="text-slate-500 font-normal text-lg leading-relaxed">
              Tu salud es nuestra prioridad. Esta información nos ayuda a adaptar tu plan de forma 100% segura y efectiva para ti.
            </p>
        </div>
