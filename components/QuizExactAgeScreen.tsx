...
interface QuizExactAgeScreenProps {
...
export const QuizExactAgeScreen: React.FC<QuizExactAgeScreenProps> = ({ onNext, onBack }) => {
  ...
  const isValid = age.length > 0 && parseInt(age) > 10 && parseInt(age) < 120;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-4 mt-4 mb-6 flex items-end space-x-3">
        ...
        <ProgressBar currentStep={20} totalSteps={46} />
      </div>
        ...
        <form onSubmit={handleSubmit} className="w-full">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 focus-within:border-[#4DB6AC] focus-within:shadow-md transition-all duration-300 mb-8">
             <input
                ...
                placeholder="Ej. 37"
                className="w-full text-center text-3xl text-[#004D40] font-bold placeholder-slate-300 outline-none bg-transparent"
                autoFocus
             />
          </div>
