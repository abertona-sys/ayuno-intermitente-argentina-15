...
interface QuizFastingKnowledgeScreenProps {
  gender: Gender;
  onNext: (knowledge: string) => void;
  onBack: () => void;
}
...
export const QuizFastingKnowledgeScreen: React.FC<QuizFastingKnowledgeScreenProps> = ({ gender, onNext, onBack }) => {
  const options = [
    { id: 'beginner', label: 'Es nuevo para mí', emoji: '🤔' },
    { id: 'intermediate', label: 'Sé un poco al respecto', emoji: '🤓' },
    { id: 'expert', label: gender === 'female' ? 'Soy una experta' : 'Soy un experto', emoji: '😎' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-4 mt-4 mb-6 flex items-end space-x-3">
        ...
        <ProgressBar currentStep={8} totalSteps={46} />
      </div>
      ...
      <div className="w-full max-w-md px-6 flex flex-col items-center text-center flex-grow pb-8">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold text-[#004D40] leading-tight mb-8">
          ¿Conocés el método ayuno intermitente a la Argentina?
        </h2>
