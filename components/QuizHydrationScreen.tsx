...
interface QuizHydrationScreenProps {
  onNext: (hydration: string) => void;
  onBack: () => void;
}
...
export const QuizHydrationScreen: React.FC<QuizHydrationScreenProps> = ({ onNext, onBack }) => {
  const options = [
    { id: 'almost_nothing', label: 'Casi nada (solo café, té o gaseosas)', emoji: '☕' },
    { id: '1_2_glasses', label: '1 - 2 vasos', emoji: '💧' },
    { id: '3_5_glasses', label: '3 - 5 vasos (aprox. 1 litro)', emoji: '🥛' },
    { id: '6_8_glasses', label: '6 - 8 vasos (aprox. 2 litros)', emoji: '🚰' },
    { id: 'more_8_glasses', label: 'Más de 8 vasos', emoji: '🌊' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-4 mt-4 mb-6 flex items-end space-x-3">
        ...
        <ProgressBar currentStep={31} totalSteps={46} />
      </div>
      ...
      <div className="w-full max-w-md px-6 flex flex-col items-center text-center flex-grow pb-8">
        ...
        <div className="flex flex-col space-y-4 w-full">
          {options.map((option) => (
            ...
            >
              <span className="text-[#004D40] font-medium text-lg sm:text-xl text-left group-hover:scale-[1.02] transition-transform">
                {option.label}
              </span>
