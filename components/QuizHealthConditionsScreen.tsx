...
interface QuizHealthConditionsScreenProps {
...
export const QuizHealthConditionsScreen: React.FC<QuizHealthConditionsScreenProps> = ({ onNext, onBack }) => {
  const [selectedConditions, setSelectedConditions] = useState<string[]>([]);
  const buttonRef = useRef<HTMLDivElement>(null);

  const options = [
    { id: 'diabetes', label: 'Diabetes (Tipo 1 o 2)', emoji: '🩸' },
    { id: 'insulin_resistance', label: 'Resistencia a la insulina / Prediabetes', emoji: '⚠️' },
    { id: 'hypothyroidism', label: 'Hipotiroidismo / Hashimoto', emoji: '🦋' },
    { id: 'pcos', label: 'Síndrome de Ovario Poliquístico (SOP)', emoji: '🌸' },
    { id: 'hypertension', label: 'Presión arterial alta', emoji: '💓' },
    { id: 'cholesterol', label: 'Colesterol alto', emoji: '🍔' },
    { id: 'gastritis', label: 'Gastritis o Reflujo', emoji: '🔥' },
    { id: 'none', label: 'Ninguna de las anteriores', emoji: '❌' },
  ];
  ...
  const handleToggle = (id: string) => {
    if (id === 'none') {
      ...
      let newSelection = selectedConditions.filter(m => m !== 'none');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-4 mt-4 mb-6 flex items-end space-x-3">
        ...
        <ProgressBar currentStep={33} totalSteps={46} />
      </div>
      ...
      <div className="w-full max-w-md px-6 flex flex-col items-center text-center flex-grow pb-12">
        ...
        <p className="text-slate-500 text-sm sm:text-base font-normal mb-8 max-w-[95%]">
          Tu salud es nuestra prioridad. Esto nos ayuda a personalizar tu plan de forma segura.
        </p>
        ...
        {selectedConditions.length > 0 && (
          <div ref={buttonRef} className="w-full mt-8 mb-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <Button onClick={() => onNext(selectedConditions)}>
              Continuar
            </Button>
          </div>
        )}
