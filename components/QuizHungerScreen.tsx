...
interface QuizHungerScreenProps {
  onNext: (hunger: string) => void;
  onBack: () => void;
}
...
export const QuizHungerScreen: React.FC<QuizHungerScreenProps> = ({ onNext, onBack }) => {
  const options = [
    { id: 'afternoon_slump', label: 'El bajón de energía a la tarde', emoji: '📉' },
    { id: 'boredom_stress', label: 'Comer por aburrimiento o estrés', emoji: '🥱' },
    { id: 'bloating_after_eating', label: 'Me hincho mucho después de comer', emoji: '🐡' },
    { id: 'no_schedule', label: 'No tengo horarios fijos y vivo improvisando', emoji: '🌪️' },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-6 flex flex-col items-center text-center flex-grow pb-8">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold text-[#004D40] leading-tight mb-8">
          Te entiendo perfecto. ¿Cuál sentís que es tu mayor obstáculo hoy en el día a día?
        </h2>
