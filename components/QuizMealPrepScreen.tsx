...
interface QuizMealPrepScreenProps {
  onNext: (prep: string) => void;
  onBack: () => void;
}
...
export const QuizMealPrepScreen: React.FC<QuizMealPrepScreenProps> = ({ onNext, onBack }) => {
  const options = [
    { id: 'cook_daily', label: 'Cocino en casa casi siempre', emoji: '🍳' },
    { id: 'cook_often', label: 'Cocino algunas veces por semana', emoji: '🥗' },
    { id: 'buy_food', label: 'Suelo pedir delivery o comer fuera', emoji: '🛵' },
    { id: 'others_cook', label: 'Alguien más cocina para mí', emoji: '🍽️' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-4 mt-4 mb-6 flex items-end space-x-3">
        ...
        <ProgressBar currentStep={14} totalSteps={46} />
      </div>
