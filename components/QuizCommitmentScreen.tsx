...
interface QuizCommitmentScreenProps {
  gender: Gender;
...
export const QuizCommitmentScreen: React.FC<QuizCommitmentScreenProps> = ({ gender, onNext, onBack }) => {
  // Ordered from Low to High commitment as per design specs
  const options = [
    { id: 'low', label: 'Me gustaría probar y ver qué pasa', emoji: '👀' },
    { id: 'medium', label: 'Quiero mejorar mi salud y perder peso', emoji: '💪' },
    { id: 'high', label: gender === 'female' ? 'Estoy totalmente decidida a lograrlo' : 'Estoy totalmente decidido a lograrlo', emoji: '🔥' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-4 mt-4 mb-6 flex items-end space-x-3">
        ...
        <ProgressBar currentStep={18} totalSteps={46} />
      </div>
      ...
      <div className="w-full max-w-md px-6 flex flex-col items-center text-center flex-grow pb-8">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold text-[#004D40] leading-tight mb-8">
          ¿Qué tan decidido estás a alcanzar tu peso ideal?
        </h2>
        ...
        <div className="flex flex-col space-y-4 w-full mb-8">
          {options.map((option) => (
            ...
            >
              <span className="text-[#004D40] font-medium text-lg sm:text-xl text-left group-hover:scale-[1.02] transition-transform max-w-[85%]">
                {option.label}
              </span>
