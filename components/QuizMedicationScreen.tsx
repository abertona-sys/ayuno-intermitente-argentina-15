...
interface QuizMedicationScreenProps {
...
export const QuizMedicationScreen: React.FC<QuizMedicationScreenProps> = ({ onNext, onBack }) => {
  const options = [
    { id: 'yes', label: 'Sí' },
    { id: 'no', label: 'No' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E0F7FA] to-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md px-4 mt-4 mb-6 flex items-end space-x-3">
        ...
        <ProgressBar currentStep={32} totalSteps={46} />
      </div>
      ...
      <div className="w-full max-w-md px-6 flex flex-col items-center text-center flex-grow pb-8">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold text-[#004D40] leading-tight mb-8">
          ¿Tomas medicamentos o suplementos regularmente?
        </h2>
        ...
        <div className="flex flex-col space-y-4 w-full">
          {options.map((option) => (
            <button 
              ...
              className="w-full bg-white py-6 px-4 rounded-2xl shadow-sm border border-slate-100 hover:border-[#4DB6AC] hover:shadow-md hover:bg-slate-50 transition-all duration-200 text-center group"
            >
              <span className="text-[#004D40] font-medium text-lg sm:text-xl block group-hover:scale-[1.02] transition-transform">
                {option.label}
              </span>
            </button>
          ))}
